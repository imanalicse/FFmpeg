...
Cut vide:
ffmpeg -ss 00:00:00 -to 00:15:00 -i Deakin.mp4 -acodec copy -vcodec copy Deakin_15_min.mp4


ffprobe -i Deakin_University_Graduation_Ceremony_4-livestream-234929152-hls-4756.mp4 -show_entries format=duration -v quiet -of csv="p=0"



public static function muteStartAndEndBannerVideoPart($input_video, $output_video, $duration_in_second, $row_id = 0) {
	...
	$total_duration = shell_exec($total_duration_cmd);
	$total_duration = floatval($total_duration);

	$second_mute_start_time = $total_duration - $duration_in_second;
	$command = 'ffmpeg -i '. $input_video .' -af "volume=enable=\'between(t,0,' . $duration_in_second . ')\':volume=0, volume=enable=\'between(t,' . $second_mute_start_time . ',' . $total_duration . ')\':volume=0" ' . $output_video . ' 2> ' .  self::commandOutputFile('muteStartAndEndBannerVideo.txt', $row_id);
	CommonService::shellExe($command);
}


ffmpeg -i input.mp4 -i logo.png -filter_complex "[1:v]scale=100:-1[logo];[0:v][logo]overlay=10:10" -c:a copy output.mp4
